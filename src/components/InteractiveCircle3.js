    ...
    const ITEM = dataPage.multimediaCircular.map( (item, i) => {
      return(
        <div className = 'circleItems' key = { i } >
          <button className = { 'circleButton ' + ( i + 1 !== 1 ? 'disabled2' : '')} id = { i + 1 } onClick = { this.enableItem }>{ item.text }</button>
        </div>
      );
    } );
    return ITEM;
      ...
      <div className = 'interactiveCircle2 d-Flex j-Ar aI-C bg-StoyrLine'>
        {
          ...
          <div className = 'bgItemGlobe'>
            <div
              ...
              <h3 className = 'mB-05 color-2'>{ multimediaCircular[actualItem - 1].itemInfo.title }</h3>
